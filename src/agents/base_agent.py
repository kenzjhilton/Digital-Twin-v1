"""
Simple Base Supply Chain Agent - Improved Version
...
class BaseSupplyChainAgent(ABC):
    ...
    def __init__(self, agent_id: str, name: str, capacity: float):
        ...
        # Sanity check (use logging instead of print)
        logger.info(f"Agent {self.name} initialized with ID {self.agent_id} and capacity {self.capacity} kg.")
    ...
    def add_connection(self, agent_id: str):
        ...
        logger.info(f"Agent {self.name} connected to agent {agent_id}.")
    ...
    def get_status(self) -> Dict:
        ...
        """
        # Format created_at for easier reading
